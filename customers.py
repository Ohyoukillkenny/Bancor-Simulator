...
class Customer(object):
    ...
    def changeValuation(self, newValuation, marketPrice):
        ...
        if self._market.ifFinishedOrder(self):
            ...
            # # half-in policy
            # if self._valuation > marketPrice and int(0.5 * self._reserveBalance) > 0:
            #     # XXX issues a buy order 
            #     self._market.buy(self, int(0.5 * self._reserveBalance)) 
            # elif self._valuation < marketPrice and self._tokenBalance> 0:
            #     # XXX issue a sell order
            #     self._market.sell(self, int(0.5 * self._tokenBalance)) 
            # else:
            #     # nothing to do
            #     pass
